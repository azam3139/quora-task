	...
	if($result) {
		...
		if(mysqli_num_rows($result) > 0) {
			if($row = mysqli_fetch_array($result)) {
				//if already has a downvote
				if($row["value"] == -1) {
					$sql = "UPDATE Votes SET value=0 WHERE user_id=" . $user_id . " AND answer_id=" . $answer_id;
					$conn->query($sql);
					$sql = "UPDATE Answers SET downvotes=downvotes-1 WHERE id=" . $answer_id;
					$conn->query($sql);
					//echo "Vote changed from downvoted";
				}
				//has no vote
				else if ($row["value"] == 0) {
				...
				//has an upvote
					...
					$sql = "UPDATE Votes SET value=-1 WHERE user_id=" . $user_id . " AND answer_id=" . $answer_id;
					$conn->query($sql);
					$sql = "UPDATE Answers SET downvotes=downvotes+1, upvotes = upvotes-1 WHERE id=" . $answer_id;
					$conn->query($sql);
					//echo "Vote changed from upvote";
				}
